...
export default function VideoWithhooks(props) {
  ...
  useEffect(() => {
    RtcEngine.on('userJoined', (data) => {
      console.log('data returned', peerIds, '=>', data);
      if (peerIds.indexOf(data.uid) === -1) {     //If new user has joined
        setPeerIds(peerIdsNew => {
          console.log('peerIdsNew', peerIdsNew);
          return [...peerIdsNew, data.uid];
        })       //add peer ID to state arr
      }
    });
    RtcEngine.on('userOffline', (data) => {
      setPeerIds(peerIdsNew => {
        return peerIdsNew.filter(uid => uid !== data.uid)
    ...
    console.log('channelId, uid', channelId, uid);
